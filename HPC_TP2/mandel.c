...
/* Allocation mémoire */
 ...
 if(rank == 0) {
  ...
  /* Allocation memoire du sous tableau à traiter */  
  ima = (unsigned char *)malloc( w*hP*sizeof(unsigned char));
 }
...
if(rank == 0) {
  for(int source = 1 ; source < np ; source++) {
    MPI_Recv(ima+source*hP*w , hP*w , MPI_CHAR , source , tag , MPI_COMM_WORLD , &status);
  }
  ...
  MPI_Send(ima , hP*w , MPI_CHAR , 0 , tag , MPI_COMM_WORLD);
}
