...
int main(int argc, char *argv[]) {
  ...
  unsigned char *local_data, *local_data_extented;
  ...
  if(rank == 0) {
    ...
    if(h%NP != 0) {
      fprintf(stderr , "NP must divide h");
      exit(-1);
    }
    h_local = h/NP;
  }
  
  // Partager les dimensions de chaque bloc de lignes 
  MPI_Bcast(&w , 1 , MPI_INT , 0 , MPI_COMM_WORLD);
  MPI_Bcast(&h_local , 1 , MPI_INT , 0 , MPI_COMM_WORLD);

  int local_data_h = h_local + (rank!=0) + (rank!=NP-1);
  local_data = (unsigned char*) malloc(sizeof(unsigned char)*local_data_h*w);

  // Partager les bloc de données de l'image
  MPI_Scatter(r.data , h_local*w , MPI_CHAR , local_data + (w*(rank!=0)) , h_local*w , MPI_CHAR , 0 , MPI_COMM_WORLD);
  ...
  for(i=0 ; i < nbiter ; i++){
    ...
    if(rank!=NP-1) {
      MPI_Sendrecv(local_data + (local_data_h-2)*w , w , MPI_CHAR , rank+1 , tag , local_data + (local_data_h-1)*w , w , MPI_CHAR , rank+1 , tag , MPI_COMM_WORLD , &status);
    }

    if(rank!=0) {
      MPI_Sendrecv(local_data + w, w , MPI_CHAR , rank-1 , tag , local_data, w , MPI_CHAR , rank-1 , tag , MPI_COMM_WORLD , &status);
    }

    convolution( filtre, local_data, local_data_h, w);
  } /* for i */

  // Rassembler tous les fragments
  MPI_Gather(local_data , h_local*w , MPI_CHAR , r.data + h_local*w*rank , h_local*w , MPI_CHAR , 0 , MPI_COMM_WORLD);
